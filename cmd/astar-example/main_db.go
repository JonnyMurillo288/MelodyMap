...
func main() {
	fmt.Println("=== A* Search with Node2Vec Embeddings (Database) ===\n")
	...
	fmt.Println("Loading embeddings from artist_embeddings_n2v table...")
	err = search.InitEmbeddingsFromDB(store)
	if err != nil {
		log.Fatalf("Failed to load embeddings from database: %v", err)
	}
	fmt.Println("✓ Embeddings loaded successfully from database\n")
